	...
	if(jQuery.yyucobjnow<jQuery.yyucobjnum){
		...
		if(eval('"undefined" == typeof '+obj)){
			if(obj=='yyuccalendar'){
				...
				getStyle('/scripts/jquery/date/mobiscroll.css');
				getScript('/scripts/jquery/date/mobiscroll.js',function(){
					jQuery.yyucobjnow = jQuery.yyucobjnow +1;
...
function tusi(txt,fun){
	$('.tusi').remove();
	var div = $('<div style="background: url(/win360/luckyturn/img/tusi.png);max-width: 85%;min-height: 77px;min-width: 270px;position: absolute;left: -1000px;top: -1000px;text-align: center;border-radius:10px;"><span style="color: #ffffff;line-height: 77px;font-size: 23px;">'+txt+'</span></div>');
	$('body').append(div);
